...
- **AND** 包含列: Name, Status, Data Sources 数量, Routes 数量, Created At
- **AND** 支持分页
- **AND** 提供搜索过滤功能
...
- **THEN** 弹出 Modal 表单
- **AND** 表单包含: Name, Description, Status 选择
- **AND** 路由规则编辑区域（可添加/删除/排序路由规则）
- **AND** 每条路由可配置: Name, File Filter (extensions/mime_types), Pipeline 选择, Target 多选

#### Scenario: 编辑 Workflow

- **WHEN** 用户点击列表中的 Workflow 行
- **THEN** 弹出预填充的编辑 Modal
- **AND** 用户可修改所有字段
