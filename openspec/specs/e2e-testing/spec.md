...
> Playwright E2E 测试覆盖 — Agent 模块 UI 功能验证

## 1. 概述

使用 Playwright 对 Agent 模块的关键 UI 功能进行端到端测试覆盖，确保功能在后续迭代中不会回退。

### 测试基础设施

- **框架**: Playwright (chromium)
- **配置**: `frontend/e2e/playwright.config.ts`
- **基础 URL**: `http://localhost:15173`
- **测试数据**: 通过 Playwright `request` API 调用 backend REST API 自管理（beforeAll 创建、afterAll 清理）
- **容错**: 前置条件不满足时使用 `test.skip()` 降级
...
| `agent-playground.spec.ts` | Playground 加载、URL 参数、按钮高亮 |
| `editor-layout.spec.ts` | 编辑器布局完整性 |

## 2. Agent History 表格 E2E 测试覆盖
