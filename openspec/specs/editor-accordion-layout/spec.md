# Editor Accordion Layout Specification

## Purpose

编辑器页面（SkillEditor、PipelineEditor、BuiltinSkillEditor）的右侧面板统一交互模式。

### Requirement: 编辑器右侧互斥面板

编辑器页面的右侧面板 SHALL 使用互斥面板模式 — Config 和 Agent 共享同一列，一次只显示一个。
...
PageHeader 中的 Agent 按钮 SHALL 控制右侧面板中 Agent Assistant 的展开。

#### Scenario: 点击 Agent 按钮展开 Agent section

- **WHEN** 用户点击 PageHeader 中的 Agent 按钮
- **AND** Agent Assistant 面板处于隐藏状态
- **THEN** Agent Assistant 面板展开，Config 面板隐藏

#### Scenario: 再次点击 Agent 按钮折叠

- **WHEN** 用户点击 PageHeader 中的 Agent 按钮
- **AND** Agent Assistant 面板已处于展开状态
- **THEN** Agent Assistant 面板隐藏，Config 面板恢复显示

### Requirement: Agent Assistant 面板内滚动

#### Scenario: 对话区域滚动

- **WHEN** Agent Assistant 面板展开
